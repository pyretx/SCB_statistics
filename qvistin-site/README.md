...
├── Dockerfile               # 2-stage: render dist/ from content.toml, then serve via nginx
├── nginx.conf               # gzip + cache + security headers (inside the container)
├── dist/                    # BUILD OUTPUT (git-ignored) — regenerated by build.py / the image build
└── README.md
```
...
- **Deploy:** a small `nginx:alpine` container serves the site via Traefik on
  `qvist.in` (+ `www`). The image **builds `dist/` from `content.toml` at build
  time** (`Dockerfile`), so `dist/` is git-ignored and you never ship a stale page —
  just edit `content.toml`, commit, and redeploy. Compose file:
  `deploy/docker-compose.qvistin-site.yml`. Full runbook + DNS/TLS: `docs/qvistin-hosting.md`.

  On the server:
  ```bash
  git clone -b dev https://github.com/pyretx/SCB_statistics.git /srv/qvistin-site
  cd /srv/qvistin-site/deploy
  docker compose -f docker-compose.qvistin-site.yml up -d --build --force-recreate
  ```
  (Traefik issues the TLS cert once qvist.in resolves to the server — i.e. after the
  one.com DNS records are added. Until then the container is up but serves no public
  traffic.)
```
