"""Japan provider — e-Stat Basic Survey on Wage Structure (table 0003426334).

Reads japan_earnings.json.gz: mean scheduled monthly cash earnings (JPY) by 11
JSCO-2020 major occupation groups × sex × year (2020–2023). Flat (major groups),
mean + sex + trend. EN + JA (native) names.
"""
...
_DATA = os.path.join(_ROOT, "japan_earnings.json.gz")
...
class JapanProvider(CountryProvider):
    ...
    def trend(self, *, sector="", occ_codes=(), sex="total", years=(),
        ...
        for y in years:
            ...
            for occ in occ_codes:
                v = data.get(occ)
                rows.append({"country": "japan", "year": int(y),
