"""Spain provider — INE Encuesta de Estructura Salarial (table 70672).

Reads spain_earnings.json.gz: gross monthly earnings (EUR, = official annual / 12)
with the full distribution — mean, P10, P25, median, P75, P90 — by CNO-11 (ISCO-08)
occupation (major + sub-major, 2 levels) × sex. EN (ISCO titles) + ES (native).
"""
...
_DATA = os.path.join(_ROOT, "spain_earnings.json.gz")
...
class SpainProvider(CountryProvider):
    ...
    def occupation_stats(self, *, sector="", occ_codes=(), sex="total", years=(),
        ...
        for occ in occ_codes:
            ...
            if not v:
                continue
            rows.append({
                "country": "spain", "year": yr, "occ_code": occ,
    ...
    def trend(self, *, sector="", occ_codes=(), sex="total", years=(),
        ...
        for y in years:
            ...
            for occ in occ_codes:
                v = data.get(occ)
                rows.append({"country": "spain", "year": int(y),
