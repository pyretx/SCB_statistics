...
def _check_brazil_data() -> SourceStatus:
    ...
    info = brbuild.bundled_info()
    yrs = info.get("years") or []
    span = f"{min(yrs)}–{max(yrs)}" if yrs else str(info.get("year", "—"))
    s = SourceStatus("brazil_data", "Brazil", "IBGE",
                     current=f"{span} · built {info.get('built_at', '—')}")
    s.latest = span
    s.update_available = False
    s.note = _notes().get("brazil_data", "Bundled IBGE snapshot — rebuild re-fetches "
                          "PNAD Contínua table 9457 from the open SIDRA API.")
    return s


def _update_brazil_data(status, log) -> UpdateResult:
    from countries.brazil import build as brbuild
    res = brbuild.build(log=log)
    if res.get("codes", 0) < 5:
        return UpdateResult("brazil_data", OUT_VALIDATION, f"too few groups: {res}")
    try:
        from countries.brazil import provider as brprov
        brprov._load.clear()
    except Exception:
        pass
    return UpdateResult("brazil_data", OUT_UPDATED,
                        f"{res['built_at']} · {res['codes']} groups, "
                        f"{len(res['years'])} years · {_commit_note('brazil_earnings.json.gz')}")


# ── Slovenia · SURS PxWeb 0711335S — bundled snapshot (per-year fetch) ────────
def _check_slovenia_data() -> SourceStatus:
    from countries.slovenia import build as sibuild
    info = sibuild.bundled_info()
    yrs = info.get("years") or []
    span = f"{min(yrs)}–{max(yrs)}" if yrs else str(info.get("year", "—"))
    s = SourceStatus("slovenia_data", "Slovenia", "SURS",
                     current=f"{span} · built {info.get('built_at', '—')}")
    s.latest = span
    s.update_available = False
    s.note = _notes().get("slovenia_data", "Bundled SURS snapshot — rebuild "
                          "re-fetches table 0711335S from the PxWeb API.")
    return s


def _update_slovenia_data(status, log) -> UpdateResult:
    from countries.slovenia import build as sibuild
    res = sibuild.build(log=log)
    if res.get("codes", 0) < 100:
        return UpdateResult("slovenia_data", OUT_VALIDATION, f"too few SKP codes: {res}")
    try:
        from countries.slovenia import provider as siprov
        siprov._load.clear()
    except Exception:
        pass
    return UpdateResult("slovenia_data", OUT_UPDATED,
                        f"{res['built_at']} · {res['codes']} codes ({res['leaves']} "
                        f"occupations), {len(res['years'])} years · "
                        f"{_commit_note('slovenia_earnings.json.gz')}")


# ── Switzerland · FSO STAT-TAB cube _205 — bundled snapshot (LSE percentiles) ──
def _check_switzerland_data() -> SourceStatus:
    from countries.switzerland import build as chbuild
    info = chbuild.bundled_info()
    yrs = info.get("years") or []
    span = f"{min(yrs)}–{max(yrs)}" if yrs else str(info.get("year", "—"))
    s = SourceStatus("switzerland_data", "Switzerland", "FSO",
                     current=f"{span} · built {info.get('built_at', '—')}")
    s.latest = span
    s.update_available = False
    s.note = _notes().get("switzerland_data", "Bundled FSO LSE snapshot — rebuild "
                          "re-fetches cube px-x-0304010000_205 from the FSO PxWeb API.")
    return s


def _update_switzerland_data(status, log) -> UpdateResult:
    from countries.switzerland import build as chbuild
    res = chbuild.build(log=log)
    if res.get("codes", 0) < 30:
        return UpdateResult("switzerland_data", OUT_VALIDATION, f"too few ISCO codes: {res}")
    try:
        from countries.switzerland import provider as chprov
        chprov._load.clear()
    except Exception:
        pass
    return UpdateResult("switzerland_data", OUT_UPDATED,
                        f"{res['built_at']} · {res['codes']} codes ({res['leaves']} "
                        f"occupations), {len(res['years'])} years · "
